...
> **Bal Defterim'in mevcut durumu (2026-07-27 — YAPILDI):** Bal Defterim'de tüm veri kişiseldir (`bd_*` tabloları hepsi `user_id → auth.users` FK'li, RLS ile `auth.uid()`; topluluk içeriği/CASCADE tuzağı YOK), bu yüzden "Silinmiş Kullanıcı" devri gerekmez. Uygulanan çözüm: (1) Ayarlar → Hesap kartına `🗑️ Hesabımı Sil` düğmesi + `onaySor` ile iki aşamalı "GERİ ALINAMAZ" onayı, ardından `bd-delete-account` fonksiyonu çağrılıp `localStorage` (`bd_cache`, `bd_kur_gosterildi`) temizlenip `signOut` + reload; (2) silme işi `bd-delete-account` adlı **Supabase Edge Function** üzerinden yapılıyor (`sb.functions.invoke('bd-delete-account')`, `verify_jwt=true`, kod: `supabase/functions/bd-delete-account/index.ts`, projeye deploy edildi — v1 ACTIVE); (3) web'den silme talebi için gizlilik sayfasına e-posta yöntemi eklendi.
>
> **PAYLAŞILAN PROJE TUZAĞI:** `pdxnpnlwrtswwifevlil` projesi Bal Defterim dışında birçok uygulamayla (`borc_*` Borç Defteri, `hediye_*`, `brkt_*` Bereket, `user_data`) **aynı `auth.users`'ı paylaşıyor** — bu yüzden Borç Defteri'nin `delete-account` fonksiyonundan ayrı, kendi `bd-delete-account` fonksiyonumuz var. "Akıllı silme": fonksiyon her zaman `bd_*` verisini siler (FK sırası: önce `bd_ayarlar/siparisler/hasatlar/zekatlar`, sonra `bd_sezonlar`), sonra `OTHER_APP_TABLES` listesinde (diğer uygulama tabloları) kullanıcının verisi var mı bakar — **varsa** auth hesabı korunur (`account_deleted:false`), **yoksa** auth hesabı tümüyle silinir (`account_deleted:true`). İstemci mesajı bu alana göre değişir. **Yeni uygulama eklenince önekini `OTHER_APP_TABLES`'a ekle.**
...
> **Bal Defterim'in mevcut durumu (2026-07-27 — YAPILDI):** `gizlilik-politikasi.html` depoya eklendi (bal/amber temalı statik sayfa). İçerik: Google OAuth ad/e-posta/foto'nun hesap işlevi için alındığı, konum/reklam kimliği toplanmadığı, verilerin Supabase'de RLS ile saklandığı (yalnız kullanıcı kendi kaydını görür), üçüncü tarafla paylaşılmadığı/satılmadığı, HTTPS aktarımı + çevrimdışı yerel önbellek, uygulama içinden hesap silme + `saimkamil@gmail.com`'a e-posta ile silme talebi, paylaşılan hesap notu, iletişim. Push edildikten sonra yayın URL'si: `https://kamilsaim.github.io/baldefterim/gizlilik-politikasi.html` — Data Safety formunda ve Store listing'de bu URL kullanılır. Ayarlar → Hesap kartından da link veriliyor.
...
- [x] Hesap silme: uygulama içi "Hesabımı sil" düğmesi + `bd-delete-account` Edge Function (akıllı silme); web'den e-posta ile talep. ✔ (index.html + Supabase'e dağıtıldı — v1 ACTIVE)
- [x] Gizlilik politikası sayfası (`gizlilik-politikasi.html`) hazır — GitHub Pages'e push edilince yayında olacak. ✔
